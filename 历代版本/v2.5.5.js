// ==UserScript==
// @name         M3U8 嗅探 + MediaGo 投喂器 v2.5.5  终于没问题版本
// @version      2.5.5
// @description  基于 v2.4.6 真经复刻 | 找回 480P/720P 全画质彩色勋章 | 强化三态动态文案反馈 | 锁定原始长链接显示与底部 API/URL 切换
// @author       zhecydn
// @match        *://*/*
// @allframes    true
...
(function() {
    ...
    function addUrl(url, customTitle = null, isBiliBatch = false) {
        ...
        if (window.self !== window.top) {
            window.top.postMessage({ type: 'VIDEO_MSG_V255', url, customTitle, isBiliBatch }, '*');
            return;
        }
    ...
    function renderSingleItem(item) {
        ...
        // 【核心继承】100% 保持 2.4.6 的原始网址长链接显示，不做任何截断
    ...
    function sendTask(url, btn, customName = null, forceBili = false) {
        ...
        if (btn) {
            btn.innerText = "⏳ 投喂中...";
            btn.style.background = "#f1c40f"; // 变黄
        ...
        const successAction = () => {
            if (btn) {
                btn.innerText = "✅ 已成功投喂";
                btn.style.background = "#27ae60"; // 变绿
    ...
    if (isMinimized) createGear(); else createPanel();
    window.addEventListener('message', e => { if (e.data && e.data.type === 'VIDEO_MSG_V255') addUrl(e.data.url, e.data.customTitle, e.data.isBiliBatch); });
})();